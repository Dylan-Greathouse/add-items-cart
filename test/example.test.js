...
import { findById, getTotal } from '../utils.js';
...
test('Is this a Potion?', (expect) => {
    ...
    const expected = `<li><h3>Potion</h3><img src="./assets/Potion.PNG" alt="Potion"><span>30 munnyz</span><h4>A bitter medicine that is used by many adventures. Heals 250 hp.</h4><button value="1" class="add">Buy</button><h5 class="qty" id="1">0</h5></li>`;
...
test('Is this a Hi potion?', (expect) => {
    //Arrange
    // Set up your arguments and expectations
    const expected = `<li><h3>Hi-potion</h3><img src="./assets/Hi-potion.PNG" alt="Hi-potion"><span>60 munnyz</span><h4>A bitter medicine, better than that boring, old potion. Heals 1000 hp.</h4><button value="2" class="add">Buy</button><h5 class="qty" id="2">0</h5></li>`;
...
test('Is this a X potion?', (expect) => {
    //Arrange
    // Set up your arguments and expectations
    const expected = `<li><h3>X-potion</h3><img src="./assets/X-potion.PNG" alt="X-potion"><span>120 munnyz</span><h4>A great potion for long journeys. A perfect potion made from years of research and perfecting the recipe. Can't change that bitter taste though. Heals 2500 hp.</h4><button value="3" class="add">Buy</button><h5 class="qty" id="3">0</h5></li>`;
...
test('Is this a Fire bomb?', (expect) => {
    //Arrange
    // Set up your arguments and expectations
    const expected = `<li><h3>Fire bomb</h3><img src="./assets/Bomb.PNG" alt="Fire bomb"><span>40 munnyz</span><h4>Throw the bomb at an enemy to inflict fire damage. Inflicts 500 fire damage to a single target.</h4><button value="6" class="add">Buy</button><h5 class="qty" id="6">0</h5></li>`;
...
test('returns fruits by id', expect => {
    ...
    const actual = findById(products, 7);
...
test('renderTableRow returns a <tr> element', expect => {
    ...
    const expected = `<tr><td>Potion</td><td>30 Munnyz</td><td>2</td><td>60 Munnyz</td></tr>`;
    const dom = renderTableRow(potion, potionCart);
    const actual = dom.outerHTML;
    expect.equal(actual, expected);
});

test('Does get potion expected 60 when qty is 2?', expect=>{
    const products = [{
        id: 1,
        name: 'Potion',
        image: 'Potion.PNG',
